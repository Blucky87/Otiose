...
namespace Nez
{
    ...
    public class PathBuilder
    {
        ...
        /// </summary>
        /// <param name="pathBuilder">An existing path.</param>
        public void AddPath (PathBuilder pathBuilder)
        {
            if (pathBuilder._geometryIndex == 0)
                return;

            if (pathBuilder._geometryIndex == 1) {
                AddPoint(pathBuilder._geometryBuffer[0]);
                return;
            }

            CheckBufferFreeSpace(pathBuilder._geometryIndex);

            int startIndex = LastPointEqual(pathBuilder._geometryBuffer[0]) ? 1 : 0;
            int baseIndex = _geometryIndex;

            for (int i = startIndex; i < pathBuilder._geometryIndex; i++)
                _geometryBuffer[_geometryIndex++] = pathBuilder._geometryBuffer[i];

            if (_calculateLengths) {
                if (pathBuilder._lengthBuffer != null) {
                    for (int i = startIndex; i < pathBuilder._geometryIndex; i++)
                        _lengthBuffer[baseIndex++] = pathBuilder._lengthBuffer[i];
                }
                else
                    CalculateLengthsInRange(baseIndex, pathBuilder._geometryIndex - baseIndex);
            }
        }
        ...
        {
            if (_geometryIndex > 0 && _geometryBuffer[0].Equals(_geometryBuffer[_geometryIndex - 1]))
            {
                AddPoint(_geometryBuffer[0]);
            }
        }
    }
}
