...
using System.Text;
using Core.svelto.ui;
using Microsoft.Xna.Framework;
using Microsoft.Xna.Framework.Input;
using Nez;
using Nez.UI;

namespace Core.svelto
{
    public class SveltoStage
    {
        ...
        SveltoTable.TableDebug debugTableUnderMouse = SveltoTable.TableDebug.None;

        Vector2 _lastMousePosition;
        SveltoElement _mouseOverElement;
        private Dictionary<int, SveltoElement> _touchOverElement = new Dictionary<int, SveltoElement>();
        List<SveltoElement> _inputFocusListeners = new List<SveltoElement>();
        ...
        ISveltoGamepadFocusable _gamepadFocusElement;


        public SveltoStage()
        {
            root = new SveltoGroup();
            root.setStage(this);
        }
        ...
        public T addElement<T>(T element) where T : SveltoElement
        {
        ...
        void drawDebug()
        {
            if (debugUnderMouse || debugParentUnderMouse || debugTableUnderMouse != SveltoTable.TableDebug.None)
            {
                ...
                if (debugTableUnderMouse == SveltoTable.TableDebug.None)
                {
                ...
                else
                {
                    ...
                    ((SveltoTable)element).tableDebug(debugTableUnderMouse);
                }

                if (debugAll && element is SveltoGroup)
                    ((SveltoGroup)element).debugAll();
        ...
        void disableDebug(SveltoElement element, SveltoElement except)
        {
            if (element == except)
                return;
            element.setDebug(false);

            if (element is SveltoGroup)
            {
                var children = ((SveltoGroup)element).children;
        ...
        void updateInputPoint(Vector2 inputPos, bool inputPressed, bool inputReleased, bool inputMoved, ref SveltoElement lastOver)
        {
        ...
        void updateInputDown(Vector2 inputPos, SveltoElement over)
        {
        ...
        void updateInputMoved(Vector2 inputPos, SveltoElement over, SveltoElement lastOver)
        {
        ...
        void handleMouseWheel(SveltoElement mouseOverElement)
        {
...
//                                });
        ...
        void updateGamepadState()
        {
            ...
            ISveltoGamepadFocusable nextElement = null;
            var direction = SveltoDirection.None;
            if (Input.gamePads[0].DpadLeftPressed || Input.gamePads[0].isLeftStickLeftPressed() || (keyboardEmulatesGamepad && Input.isKeyPressed(Keys.Left)))
                direction = SveltoDirection.Left;
                ...
                direction = SveltoDirection.Right;
                ...
                direction = SveltoDirection.Down;

            // make sure we have a valid direction
            if (direction != SveltoDirection.None)
            {
        ...
        /// </summary>
        public void removeInputFocusListener(SveltoElement element)
        {
        ...
        public List<SveltoElement> getElements()
        {
        ...
        public void setDebugTableUnderMouse(SveltoTable.TableDebug debugTableUnderMouse)
        {
        ...
        {
            setDebugTableUnderMouse(debugTableUnderMouse ? SveltoTable.TableDebug.All : SveltoTable.TableDebug.None);
        }
        ...
        public void setGamepadFocusElement(ISveltoGamepadFocusable focusable)
        {
        ...
        public SveltoElement hit(Vector2 point)
        {
        ...
        ISveltoGamepadFocusable findNextGamepadFocusable(ISveltoGamepadFocusable relativeToFocusable, SveltoDirection sveltoDirection)
        {
            ...
            for (var i = 0; i < buttons.Count; i++)
            {
                ...
                var element = buttons[i] as SveltoElement;
        ...
        void findAllElementsOfType<T>(List<SveltoElement> elements, List<T> foundElements) where T : class
        {
            for (var i = 0; i < elements.Count; i++)
            {
                if (elements[i] is T)
                    foundElements.Add(elements[i] as T);
                else if (elements[i] is SveltoGroup)
                    findAllElementsOfType<T>(((SveltoGroup)elements[i]).children, foundElements);
            }
        }
