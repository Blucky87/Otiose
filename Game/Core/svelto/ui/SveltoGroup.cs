...
{
    public class SveltoGroup : SveltoElement, ICullable
    {
        internal List<SveltoElement> children = new List<SveltoElement>();
        ...
        public T insertElement<T>(int index, T element) where T : SveltoElement
        {
            if (element.parent != null)
                element.parent.removeElement(element);
        ...
        public virtual bool removeElement(SveltoElement element)
        {
        ...
        public List<SveltoElement> getChildren()
        {
            return children;
        }
        ...
        internal override void setStage(SveltoStage stage)
        {
            this.stage = stage;
        ...
        void setLayoutEnabled(SveltoGroup parent, bool enabled)
        {
            for (var i = 0; i < parent.children.Count; i++)
            {
                if (parent.children[i] is ISveltoLayout)
                    ((ISveltoLayout)parent.children[i]).layoutEnabled = enabled;
                else if (parent.children[i] is SveltoGroup)
                    setLayoutEnabled(parent.children[i] as SveltoGroup, enabled);
            }
        }
        ...
        public override SveltoElement hit(Vector2 point)
        {
            if (touchable == SveltoTouchable.Disabled)
                return null;
        ...
        protected Matrix2D computeTransform()
        {
            ...
            SveltoGroup parentGroup = parent;
        ...
        public void setDebug(bool enabled, bool recursively)
        {
            ...
            if (recursively)
            {
                ...
                {
                    if (child is SveltoGroup)
                        ((SveltoGroup)child).setDebug(enabled, recursively);
                    else
        ...
        public virtual SveltoGroup debugAll()
        {
            setDebug(true, true);
            return this;
        }
